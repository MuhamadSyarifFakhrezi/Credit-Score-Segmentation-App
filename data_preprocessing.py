...
def data_preprocessing(data):
    ...
    transformed_Credit_Mix = encoder_Credit_Mix.transform(data[["Credit_Mix"]])[0]
    transformed_Credit_Mix_df = pd.DataFrame([transformed_Credit_Mix], columns=encoder_Credit_Mix.get_feature_names_out(['Credit_Mix']))
    df = pd.concat([df, transformed_Credit_Mix_df], axis=1)

    transformed_Payment_of_Min_Amount = encoder_Payment_of_Min_Amount.transform(data[["Payment_of_Min_Amount"]])[0]
    transformed_Payment_of_Min_Amount_df = pd.DataFrame([transformed_Payment_of_Min_Amount], columns=encoder_Payment_of_Min_Amount.get_feature_names_out(['Payment_of_Min_Amount']))    
    df = pd.concat([df, transformed_Payment_of_Min_Amount_df], axis=1)

    transformed_Payment_Behaviour = encoder_Payment_Behaviour.transform(data[["Payment_Behaviour"]])[0]
    transformed_Payment_Behaviour_df = pd.DataFrame([transformed_Payment_Behaviour], columns=encoder_Payment_Behaviour.get_feature_names_out(['Payment_Behaviour']))
    df = pd.concat([df, transformed_Payment_Behaviour_df], axis=1)
